...
- 2026-07-03: BackVora dev DB (data/linkbuilder.db) lacks the link_type_presets table: POST /api/v1/link-prices/types/add 500s (pre-existing, observed 2026-07-03); the paired /link-prices create still succeeds, so '+ New' custom types save the price but not the type.
- 2026-07-03: flow drive can spawn a fresh worktree detached at master even when the task's feature branch already exists with the REAL baton (e.g. after a rounds-exhausted baton->human handoff): before treating a dispatch as a fresh Round-1 build, check for an existing feature/<slug> branch and read ITS STATE.md — and check other worktrees for uncommitted baton edits (the human's authorization may live there).
- 2026-07-07: BackVora tests gate: python3 (pyenv 3.14.6) has no pytest in the drive env; relay_pytest_changed.sh now resolves the main worktree's venv/bin/python via git-common-dir and deselects 3 documented pre-existing master failures (lifecycle/link_monitor/article_writer) — remove deselects when those tests get fixed.
